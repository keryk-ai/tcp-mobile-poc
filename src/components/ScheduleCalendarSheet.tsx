...
const AVAIL_CONFIG = {
  high:   { label: 'High Availability',   bg: 'bg-emerald-50',  text: 'text-emerald-700',  border: 'border-emerald-200', dot: 'bg-emerald-500' },
  medium: { label: 'Medium Availability', bg: 'bg-amber-50',    text: 'text-amber-700',    border: 'border-amber-200',   dot: 'bg-amber-400'   },
  low:    { label: 'Low Availability',    bg: 'bg-gray-50',     text: 'text-gray-500',     border: 'border-gray-200',    dot: 'bg-gray-400'    },
};
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-2 border-b border-gray-100 flex-shrink-0">
          ...
          <div className="flex-1">
            <div className="font-bold text-gray-900 text-base">Schedule a Crew</div>
            <div className="text-xs text-gray-400">Select your preferred work date</div>
          </div>
        </div>

        {/* Promo banner */}
        <div className="mx-4 mt-3 px-3 py-2 rounded-xl bg-[hsl(25,100%,97%)] border border-[hsl(25,100%,85%)] flex-shrink-0">
          <p className="text-xs text-[hsl(25,100%,40%)] font-medium">
            Friday bookings receive a <strong>5% discount</strong> — AWP has high crew availability on Fridays.
          </p>
        </div>

        {/* Day list */}
        <div className="overflow-y-auto flex-1 px-4 py-3 space-y-2 pb-32">
          {days.map((day, i) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left rounded-xl border-2 p-3 transition-all ${
                  isSelected
                    ? 'border-[hsl(25,100%,50%)] bg-[hsl(25,100%,97%)]'
                    : day.isFriday
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="w-14 flex-shrink-0 text-center">
                    <div className={`text-xs font-bold uppercase tracking-wide ${day.isFriday ? 'text-[hsl(25,100%,50%)]' : 'text-gray-500'}`}>
                      {day.dayName}
                    </div>
                    <div className={`text-base font-bold mt-0.5 ${isSelected ? 'text-[hsl(25,100%,50%)]' : 'text-gray-900'}`}>
                      {day.dateStr.split(' ')[1]}
                    </div>
                    <div className="text-[10px] text-gray-400">{day.dateStr.split(' ')[0]}</div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm font-medium text-gray-800">{day.weather.label}</span>
                    </div>
                    <div className="text-xs text-gray-500 mt-0.5">
                      H {day.weather.high}° · L {day.weather.low}°
                    </div>
                    {day.weather.note && (
                      <div className="text-[10px] text-amber-600 font-medium mt-0.5">{day.weather.note}</div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    {day.isFriday && (
                      <span className="text-[10px] font-bold text-[hsl(25,100%,50%)] bg-[hsl(25,100%,95%)] border border-[hsl(25,100%,80%)] px-2 py-0.5 rounded-full">
                        5% OFF
                      </span>
                    )}
                  </div>
        ...
        {selected !== null && !confirmed && (
          <div className="absolute bottom-0 left-0 right-0 px-4 pb-safe pb-6 pt-3 bg-white border-t border-gray-100">
            <button
